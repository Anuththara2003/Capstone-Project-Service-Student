...
Student-Service manages the full lifecycle of student records — creation, retrieval, updates, and deletion — along with profile picture storage and retrieval. It registers itself with the Eureka Service-Registry and pulls its configuration from the Config-Server, making it a fully-managed member of the ECA microservices ecosystem.
...
⚠️ **Requests must use `Content-Type: multipart/form-data`.**
...
└──────┬──────────────┬──────┘
       ...
       ▼              ▼
┌────────────┐  ┌──────────────────────┐
│ PostgreSQL │  │ Local Picture Storage│
│(Port 12500)│  │ ~/.ijse/eca/students/│
└────────────┘  └──────────────────────┘
       ▲
...
⚠️ **Prerequisites:** Config-Server, Service-Registry, and Api-Gateway must be running. A PostgreSQL instance must be accessible on port 12500 with a database named `eca`.
